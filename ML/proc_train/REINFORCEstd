    ...
    with tqdm(range(iterations)) as tqd:
        for iter in tqd:
            ...
            if learning_range == 'all':
                ...
                startpoint = max(len(variables) - learning_range + random.randrange(0, hist),
                                 hist + random.randrange(0, hist))
            ...
            if iter % update_period == 0:
                ...
                EarlyStopped = False
                if iter > 128 and np.std(scores[-64:]) >= np.std(scores[-128:-64]) and np.max(scores[-64:]) <= np.max(
                        scores[-128:-64]):
                    EarlyStopped = True
                    break

                tqd.set_postfix(Time=train_size, Score=np.mean(scores[-64:]), STD=np.std(scores[-64:]),
                                MAX=np.max(scores[-64:]), Min=np.min(scores[-64:]), ES=EarlyStopped)
                iter_log.append("{} Learning  {}  Score  {}   Var  {}   Max  {}   Min  {}   ES  {}"
                                .format(train_size, iter, np.mean(scores[-64:]), np.std(scores[-64:]),
                                        np.max(scores[-64:]), np.min(scores[-64:]), EarlyStopped))
...
if __name__ == '__main__':
    ...
    maxRewardPeriod = 0.3
    learning_range = 252
    save_direct = basedir + '/weights/reinforcestd_bm/weights252'

    with open(inputdata_direct, 'rb') as f:
        arr = pickle.load(f)

    firsttime = True
    for idx in range(252, len(arr), learning_period):
        if firsttime == True:
            model_load = False
            firsttime = False
        else:
