...
def predModel(weight_dir, variables, kospi_var):
    ...
    model = pickle.load(open(weight_dir + str(756) + '.sav', 'rb'))
    memory = []
    score = 0
    startpoint = 756
    for idx in range(startpoint, len(variables) - fwd_idx, fwd_idx):
        ...
        while model_loaded == False:
            try:
                model = pickle.load(open(weight_dir + str(idx+loc_num) + '.sav', 'rb'))
            except:
                loc_num += -1
                if loc_num < -21:
                    model_loaded=True
...
weight_dir = dir + 'weights/logistics/'
...
final_port = pd.DataFrame(total_memory[2],index=main_variables[:-1].index)
