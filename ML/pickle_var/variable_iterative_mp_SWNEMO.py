...
def normalization_PCA(regex_str, sourceArray, exp_variance):
    ...
    variables.columns = ['KospeDeT'] + list(varpca.columns)
...
def process_data(idx):
    ...
    variables_all = normalization_PCA('F_|S_|E_', arr, 0.9)
    variables_excSent = normalization_PCA('F_|E_', arr, 0.9)
    variables_excFdmt = normalization_PCA('S_|E_', arr, 0.9)
    variables_excEcon = normalization_PCA('F_|S_', arr, 0.9)
    variables_onlySent = normalization_PCA('S_', arr, 0.9)
...
dir = 'C:/PycharmProjects/pythonProject/textAnalysis'
...
if __name__ == '__main__':
    ...
    a_pool.map(process_data, range(0, LoopEndCount + 1, 1))
