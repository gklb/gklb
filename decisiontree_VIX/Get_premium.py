...
if __name__ == '__main__':
    ...
    arr['classifier'] = 1
    for index in range(23, len(arr)):
        if (arr.SMB.iloc[index-22:index] + 1).prod() > 1.00 and (arr.HML.iloc[index - 22:index] + 1).prod() > 1.00:
            arr.classifier.iloc[index - 23] = 0
        elif (arr.SMB.iloc[index - 22:index] + 1).prod() > 1.00 and (arr.HML.iloc[index - 22:index] + 1).prod() <= 1.00:
            arr.classifier.iloc[index - 23] = 1
        elif (arr.SMB.iloc[index - 22:index] + 1).prod() <= 1.00 and (arr.HML.iloc[index - 22:index] + 1).prod() > 1.00:
            arr.classifier.iloc[index - 23] = 2
        elif (arr.SMB.iloc[index - 22:index] + 1).prod() <= 1.00 and (
                arr.HML.iloc[index - 22:index] + 1).prod() <= 1.00:
            arr.classifier.iloc[index - 23] = 3
        else:
            arr.classifier.iloc[index - 23] = 4
